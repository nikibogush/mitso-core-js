...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.left >= rect2Right || rect2.left >= rect1Right) {
    return false;
  }
  if (rect1.top >= rect2Bottom || rect2.top >= rect1Bottom) {
    return false;
  }
  return true;
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let col = 0; col < 3; col += 1) {
    if (position[0][col] === position[1][col] && position[1][col] === position[2][col] && position[0][col] !== '') {
      return position[0][col];
    }
  }

  if (position[0][0] === position[1][1] && position[1][1] === position[2][2] && position[0][0] !== '') {
    return position[0][0];
  }
  if (position[0][2] === position[1][1] && position[1][1] === position[2][0] && position[0][2] !== '') {
    return position[0][2];
  }
  return undefined;
}
